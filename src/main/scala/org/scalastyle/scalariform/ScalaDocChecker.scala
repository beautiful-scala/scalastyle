...
class ScalaDocChecker extends CombinedChecker {
  ...
  override def verify(ast: CombinedAst): List[ScalastyleError] = {
    ...
    val tokensToIgnore =
      getString("ignoreTokenTypes", DefaultIgnoreTokenTypes).split(",").filterNot(_.isEmpty).toSet
    ...
    if (ignore)
      ...
      localVisit(skip = false, HiddenTokens(Nil), ignoreOverride, indentStyle, ast.lines, tokensToIgnore)(
        ast.compilationUnit.immediateChildren.head
      )
    }
  }
      ...
      if (bracketDepth > 1) {
        ...
        xs match {
          ...
          case Token(Tokens.COMMA, _, _, _) :: Token(Tokens.VARID, paramName, _, _) :: t =>
            paramName :: tparams(t, bracketDepth)
          // [A ...]
          case Token(Tokens.LBRACKET, _, _, _) :: Token(Tokens.VARID, paramName, _, _) :: t
              if bracketDepth == 0 =>
  ...
  private def localVisit(
    ...
    ast match {
      case t: FullDefOrDcl =>
        ...
        val scalaDocs = for {
          tree    <- t.annotations ++ t.modifiers
          token   <- tree.tokens
        ...
        visit(
          ...
          localVisit(
            skip,
            HiddenTokens(fallback.tokens ++ scalaDocs),
            ignoreOverride,
            indentStyle,
            lines,
            tokensToIgnore
      ...
      case t: TmplDef =>
        ...
        errors ++ visit(
          t,
          localVisit(skip, NoHiddenTokens, ignoreOverride, indentStyle, lines, tokensToIgnore)
        )
      case t: FunDefOrDcl =>
        // def foo[A, B](a: Int): B = ...
        val (_, line) = lines.findLineAndIndex(t.firstToken.offset).get
...
object ScalaDocChecker {
  private val availableTokensToIgnore =
    Set(classOf[PatDefOrDcl], classOf[TypeDefOrDcl], classOf[FunDefOrDcl], classOf[TmplDef])
      .map(_.getSimpleName)
  ...
  private object ScalaDoc {
    ...
    def apply(raw: Token, offset: Int): ScalaDoc = {
      ...
      val indentStyle = {
        def getStyle(xs: List[String], style: DocIndentStyle): DocIndentStyle =
          xs match {
            case x :: tail =>
              val prefixSizeDiff =
                if (x.trim.head == '*') x.substring(0, x.indexOf("*")).length - offset else -1
      ...
      def combineScalaDocFor[A](lines: List[ScalaDocLine], tag: String, f: (String, String) => A): List[A] =
        lines match {
